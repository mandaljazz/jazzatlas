...
var bounds = [
  [7.41838, 58.011527], // Southwest coordinates
...
function initMap() {
  ...
  map.fitBounds([[7.443059, 58.024003], [7.458623, 58.028287]]);
  ...
  geolocate.on("geolocate", function(e) {
    ...
    if (locationMarker) {
      ...
      locationMarkerAccuracy = new mapboxgl.Marker(accuracyMarker, {
        offset: [-70 / 2, -70 / 2]
      })
      ...
      locationMarker = new mapboxgl.Marker(locationMarker, {
        offset: [-15 / 2, -15 / 2]
      })
        .setLngLat([e.coords.longitude, e.coords.latitude])
        .addTo(map);
...
function addEventsToMap() {
  ...
  eventsCollection.features.forEach(function(eventFeature, i) {
    ...
    eventFeature.properties.title =
      eventFeature.properties.artist + " @ " + eventFeature.properties.place;
    eventFeature.properties.imageURL = eventFeature.properties.image
      ? "images/artists/" + eventFeature.properties.image + ".jpg"
      : "images/artists/" + defaultImages[i % defaultImages.length] + ".jpg";
    eventFeature.properties.mandaljazzURL = eventFeature.properties.image
      ? "http://mandaljazz.no/artister/" + eventFeature.properties.image
      : "http://mandaljazz.no/";

    markerDiv.style.backgroundImage =
      "url(" + eventFeature.properties.imageURL + ")";
    ...
    var $popupDiv = $("<div>")
      ...
      <div class='event-info'> \
        <div class='start-time'>" +
          moment(eventFeature.properties.start).format("dddd HH:mm") +
          "</div> \
        <div class='title'>" +
          eventFeature.properties.title +
          "</div> \
        <div>Les mer på <a class='event-link' href='" +
          eventFeature.properties.mandaljazzURL +
          "' target='_blank'>mandaljazz.no</a></div> \
      </div>"
      );

    var popup = new mapboxgl.Popup().setDOMContent($popupDiv.get(0));
  ...
  $(".event-marker").click(function(e) {
    ...
    $("html, body").scrollTop(
      $("#" + e.target.id.replace("marker-", "")).position().top +
        2 -
        distanceToTopForActiveEventInfo
    );
  });
}
...
function playback(index) {
  ...
  if (isPlaying) {
    ...
    $("html, body").animate(
      {
        scrollTop: Math.max(
          $("#event-" + currentEventFeature.id).position().top +
            2 -
            distanceToTopForActiveEventInfo,
          1
        )
